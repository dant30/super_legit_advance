...
const CustomerProfile = ({ customer: customerProp, loading: loadingProp, customerId: customerIdProp } = {}) => {
  const params = useParams();
  const id = customerIdProp || params.id;
  ...
  }, [id, customerProp, fetchCustomer, getEmployment, getGuarantors]);
  ...
  if (selectedCustomerError || !customer) {
    return (
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            ...
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end space-x-3">
              ...
              <button
                ...
                disabled={isActionLoading || !blacklistReason.trim()}
                className={`px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 ${isActionLoading || !blacklistReason.trim() ? 'bg-red-400' : 'bg-red-600 hover:bg-red-700'}`}
              >
                {isActionLoading ? 'Processing...' : 'Confirm Blacklist'}
              </button>
            </div>
          </div>
        </div>
